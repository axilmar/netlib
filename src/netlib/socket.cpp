...
namespace netlib {
    ...
    //send packet
    size_t socket::send_packet(const void* buffer, size_t size, int flags) {
        size_t remaining_size = size;

        while (remaining_size > 0) {
            size_t bytes_sent = send(buffer, remaining_size, flags);
            ...
            buffer = reinterpret_cast<const char*>(buffer) + bytes_sent;
        }
    ...
    //send packet to
    size_t socket::send_packet(const void* buffer, size_t size, const socket_address& addr, int flags) {
        size_t remaining_size = size;

        while (remaining_size > 0) {
            size_t bytes_sent = send(buffer, remaining_size, addr, flags);
    ...
    //receive packet
    size_t socket::receive_packet(void* buffer, size_t size, int flags) {
        size_t remaining_size = size;

        while (remaining_size > 0) {
            size_t bytes_received = receive(buffer, remaining_size, flags);
            ...
            buffer = reinterpret_cast<char*>(buffer) + bytes_received;
        }
    ...
    //receive packet from
    size_t socket::receive_packet(void* buffer, size_t size, socket_address& addr, int flags) {
        size_t remaining_size = size;

        while (remaining_size > 0) {
            size_t bytes_received = receive(buffer, remaining_size, addr, flags);
